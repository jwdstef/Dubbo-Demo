package cn.finance.dubbo.framework.mybatis;
...
public class PagePlugin implements Interceptor {
			...
			if (mappedStatement.getId().matches(pageSqlId)) {
				...
				if (parameterObject == null) {
					...
					if (parameterObject instanceof PageSearch) {
						...
						if (pageField != null) {
							page = (PageSearch) ReflectHelper.getValueByFieldName(parameterObject, "page");
							if (page == null)
								page = new PageSearch();
							page.setTotalResult(count);
	 ...
	 * @throws SQLException
	 */
	private void setParameters(PreparedStatement ps, MappedStatement mappedStatement, BoundSql boundSql, Object parameterObject,BoundSql oldBound) throws SQLException {
		...
		if (parameterMappings != null) {
			...
			for (int i = 0; i < parameterMappings.size(); i++) {
				...
				if (parameterMapping.getMode() != ParameterMode.OUT) {
					...
					if(oldBound.hasAdditionalParameter(propertyName)){
						boundSql.setAdditionalParameter(propertyName, oldBound.getAdditionalParameter(propertyName));
					}
